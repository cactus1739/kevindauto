...
**Quy trình xuất bản — ĐÃ TỰ ĐỘNG HOÁ** (`.github/workflows/deploy.yml`):
- **Tự động:** GitHub Actions chạy `woo:sync` + build + deploy **mỗi 15 phút** (cron `*/15 * * * *`).
  Sửa admin xong, web tự lên trong tối đa 15 phút, không cần làm gì thêm.
- **Lên ngay (~2 phút):** vào https://github.com/cactus1739/kevindauto/actions/workflows/deploy.yml
  → bấm **"Run workflow"**.
- **Chạy tay từ máy (khi cần debug):**
  ```bash
  npm run woo:sync     # kéo sản phẩm mới nhất từ WooCommerce -> products.generated.ts
  npm run build        # dựng lại web
  npm run deploy       # đẩy lên kevindauto.com
  ```
